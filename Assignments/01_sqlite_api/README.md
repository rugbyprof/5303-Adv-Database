...
_user-visible_, and whether writes started failing — not p99 latency charts.
...
| Load / contention | a plain `asyncio.gather` script, or `oha` / `hey` — to _trigger_ contention, not measure it  |
...
## Phase 2 — Joins & aggregates (fine — _if_ the index exists)
...
**Answer with data:** at what write concurrency / rate does SQLite stop being appropriate _for this workload_?
...
- Re-point one endpoint at **PostgreSQL in Docker**; contrast the _concurrency
  behavior_, not the speed.
